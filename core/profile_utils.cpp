                ...
                )
{
    ...
    ss << "\nPerf " << comma_num(perf.size()) << " x " << a_title << "\n";
    if(perf.empty())
        return ss.str();
    ...
    for(double qq : {0.0, 0.10, 0.5, 0.75, 0.9, 0.99, 0.995, 0.998, 0.999, 1.0} )
        ss << format_string("  %5.1f %% : %12s cc  %7.3f us\n",
                        qq * 100, comma_num(qtl(qq)).c_str(), qtl(qq) * cc2us);
    ...
    ss << format_string("  average : %12s cc  %7.3f us\n",
            comma_num(avg).c_str(), favg * cc2us);
    return ss.str();
}
